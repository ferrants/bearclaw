...
The daemon adds: message bus (`src/bus/`) with async queues, channels (`src/channels/` — CLI), multi-agent orchestration (`src/orchestrator/` — routing, mentions, conversation fan-out/fan-in), and HTTP gateway (`src/gateway/`) with pairing auth.
...
- **One runtime dep only**: `@noble/ciphers` (encryption)
