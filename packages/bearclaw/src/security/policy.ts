...
export class SecurityPolicy {
  ...
  isCommandAllowed(command: string): boolean {
    ...
    for (const segment of normalized.split('\x00')) {
      ...
      // Workspace-only path guard for exec args (best-effort)
      if (this.workspaceOnly && !this.areCommandPathsAllowed(parts.slice(1))) {
        return false;
      }
  ...
  private areCommandPathsAllowed(args: string[]): boolean {
    for (let i = 0; i < args.length; i++) {
      const raw = args[i];
      if (!raw) continue;

      const token = this.stripQuotes(raw);

      // Handle flags with separate path arguments
      if (token === '-C' || token === '--work-tree' || token === '--git-dir') {
        const next = args[i + 1];
        if (next) {
          const nextToken = this.stripQuotes(next);
          if (this.looksLikePath(nextToken) && !this.isPathAllowed(nextToken)) return false;
        }
        i++;
        continue;
      }
      ...
      if (eqIdx > 0) {
        ...
        if (this.looksLikePath(value) && !this.isPathAllowed(value)) return false;
        continue;
      }

      if (this.looksLikePath(token) && !this.isPathAllowed(token)) return false;
    }
